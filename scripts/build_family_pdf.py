...
def footer(canvas, doc):
    ...
    canvas.drawCentredString(A4[0] / 2, 10 * mm, "Optima Fide  |  Goianul Nou, Stăuceni  |  +37379002064  |  optimafide.info@gmail.com")
    canvas.restoreState()
...
team = [
    ...
    ("assets/images/team/team-anastasia-owner.webp", "Анастасия Плевская", "Психолог / поддержка участников", "Оказывает психологическую поддержку и помогает участникам включаться в процесс восстановления."),
...
story += [*team_grid(team), PageBreak(), p("Что входит в программу", "T"), p("Программа, условия и дополнительные вопросы", "Small"), p("В базовую программу входит", "H"), *bullets(["проживание и питание", "ежедневный ритм, группы и сопровождение", "поддержка семьи и духовная поддержка по желанию", "активности на территории центра", "подготовка к реинтеграции"]), p("Обсуждается индивидуально", "H"), *bullets(["восстановительные поездки по Молдове", "массаж и парикмахер", "консультации профильных специалистов", "анализы и обследования через профильные учреждения", "индивидуальный транспорт, сопровождение и запись к специалистам"]), note("Важно", "Некоторые дополнительные вопросы обсуждаются индивидуально во время консультации."), PageBreak(), p("Спокойная смена обстановки", "T"), p("Восстановительные поездки по Молдове", "Small")]
...
story += [note("Условие", "Любые выезды обсуждаются индивидуально и организуются только по предварительному согласованию с учётом состояния участника и правил центра."), PageBreak(), p("Как начать", "T"), p("Простой путь от первого разговора до приезда", "Small")]
...
for q, a in [("Можно ли сначала просто поговорить?", "Да. Первый разговор конфиденциален и не обязывает к поступлению."), ("Есть ли проживание и питание?", "Да, они входят в резиденциальную программу."), ("Можно ли семье обратиться без самого человека?", "Да. Семья может получить ориентирование по возможным следующим шагам."), ("Есть ли медицинские консультации?", "При необходимости они организуются через профильных специалистов и учреждения.")]:
    story += [p(f"<b>{q}</b>"), p(a)]
story += [p("Контакты", "H"), p("<b>+37379002064</b>"), p("optimafide.info@gmail.com  |  Telegram: @optimafide"), p("Goianul Nou, Stăuceni, Republica Moldova", "Small")]
